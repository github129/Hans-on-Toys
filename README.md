...
リポジトリに同梱の `dist/shot.exe` をそのまま実行できます。**Python のインストール不要**です。

```
dist\shot.exe をダブルクリック → タスクトレイに常駐して起動
```

> **個人情報マスク機能**（`Ctrl+Alt+P`）を使う場合のみ、別途 [Tesseract OCR](https://github.com/UB-Mannheim/tesseract/wiki) のインストールが必要です。

---

## セットアップ（ソースから動かす・開発する場合）
